...
export default {
  ...
  //方法集合
  methods: {
    DeviceStatusFn(factoryNo) {
      this.$api.DeviceStatus({factoryNo}).then((res) => {
        console.log('当前设备返回状态',res)
        this.device = res.data.data.status;
        console.log('状态',this.device)
        this.DevicelistFn();
      });
    },
  ...
  created() {
    ...
    this.DeviceStatusFn(this.$route.query.factoryNo);
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
  mounted() {},
  //生命周期-创建之前
