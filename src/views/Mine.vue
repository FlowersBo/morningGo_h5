<template>
  <div id="mine">
    ...
    <div class="mineWrap">
      ...
      <div class="info">
        <van-cell title="所在小组："
                  style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">{{userInfoLocal.groupname}}</van-cell>
        <van-cell v-if="isTabBat"
                  title="订单列表"
                  is-link
                  to="OrderList" />
        <van-cell title="我的工单"
      ...
      <div class="btnWrap">
        <van-button round
                    color="linear-gradient(to right, #F15A24, #877974)"
                    @click="logOut">
          退出登录
        </van-button>
      </div>
      <van-popup v-model="isShow">
        <form class="subminWrap"
              report-submit
          ...
          <div class="subBtn">
            <button class="submitBtn"
                    formType="submitFn">确认</button>
            <button class="submitBtn"
                    type="button"
                    @click="showPopup">取消</button>
          </div>
        </form>
      </van-popup>
    </div>
...
export default {
  ...
  created () {
    ...
    if (this.userInfoLocal.deviceTypeIds == '1432247982589214720') {
      this.isTabBat = false,
        this.active = 1
    } else {
      this.isTabBat = true,
